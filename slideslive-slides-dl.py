...
def get_video_id(video_url):
    ids = re.findall(
        "https://slideslive\\.(com|de)/([0-9]*)/([^/]*)(.*)", video_url
    )
    if len(ids) < 1:
...
def download_slides(
    video_id, video_name, df, base_img_url, size, headers, wait_time
):
    folder_name = "{0}-{1}".format(video_id, video_name)
    for index, row in df.iterrows():
        img_url = base_img_url.format(video_id, row["slideName"], size)
        file_path = "{0}/{3}-{1}-{2}.jpg".format(
            folder_name, row["slideName"], size, row["time"]
        )
        print("downloading {}".format(file_path))
...
"""
python slideslive-slides-dl.py https://slideslive.com/38917371/the-state-of-sparsity-in-deep-neural-networks
"""
if __name__ == "__main__":
    ...
    xml = download_slides_xml(
        args.basedataurl, video_id, video_name, headers, args.waittime
    )
